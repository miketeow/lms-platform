...
export function CourseStructure({ data }: CourseStructureProps) {
  const initialItems =
    data.chapter.map((chapter) => ({
      ...
      isOpen: true, // dafault chapters to open
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    // From the sortableitemprops interface
    ...
    if (activeType === "lesson" && overType === "lesson") {
      ...
      if (!chapterId || chapterId !== overChapterId) {
        toast.error(
          "Could not move lesson between different chapters or invalid chapter ID",
        );
        return;
      }
  ...
  function SortableItem({ id, className, data, children }: SortableItemProps) {
    const {
      attributes,
      listeners,
      setNodeRef,
